        ...
        <div class="row">
          <div class="col-12">
            <div class="card">
              ...
              <a href="javascript:;" class="btn btn-block btn-warning btn-sm" data-toggle="modal" data-target="#addlocation">Add Permission</a>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
...
<div id="addlocation" class="modal modal-info fade" role="dialog">
    <div class="modal-dialog">
        ...
        <div class="modal-body">
            ...
            route('admin.savePermission') }}">
            @csrf
            <div class="box-body autocomplete_box">
                
                <div class="form-group">
                    <label class="control-label col-sm-12" for="location">Permission Name<span class="text-danger">*</span></label>
                    <div class="col-sm-12">
                        <input type="text" class="form-control input-lg" id="name" placeholder="Name" required="" name="name" >
                    </div>
                </div>
                ...
                <div class="col-sm-offset-2 col-sm-12">
                    <button type="submit" class="btn btn-block btn-warning btn-sm btn-submit">Add Permission</button>
                </div>
                </div></div>
            </form>
        </div>                    
...
$('.select2').select2();
  ...
  table_instance = $('#role').DataTable({
    ...
    ajax: {
        url: "{{route('admin.roles.permissionlist')}}",
        method: 'GET',
        data:function(d)
        {
...
$(document).on("click",".editlocation", function(){
    ...
    $.ajax({
        url: "{{route('admin.roles.getPermission')}}",
        method: "POST",                   
        data:{id:id},
 ...
 function removelocation(obj,id){
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "{{route('admin.roles.deletePermission')}}",
                method: "POST",                   
                data:{id:id},
                success: function (resultData) {
                    var msg = resultData.message;
  ...
  $('#updatelocation').validate({
        ...
        submitHandler: function(form) {         
                ...
                l.start();
                $.ajax({
                    url: "{{route('update-permission-data')}}",
                    method: "POST",                   
